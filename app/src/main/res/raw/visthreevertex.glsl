...
//    gl_Position = vec4(a_Position.x + (noise * a_DB_Level[positionIndex] * 0.025), a_Position.yzw); 	    // gl_Position is a special variable used to store the final position.
    vec4 newPosition = vec4(a_Position.x, a_Position.y + (noise * ((a_DB_Level[0]+a_DB_Level[1]+a_DB_Level[2]+a_DB_Level[3])+a_DB_Level[4]+a_DB_Level[5] / 6.0) * 0.02), a_Position.zw); 	    // gl_Position is a special variable used to store the final position.
