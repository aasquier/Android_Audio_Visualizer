...
uniform float time;
uniform float a_DB_Level[50];

void main() {           		    // The entry point for our vertex shader.
    float noise = snoise(a_Position.xy);

    gl_Position = vec4(a_Position.x + (noise * a_DB_Level[0] *0.009), a_Position.yzw); 	    // gl_Position is a special variable used to store the final position.
